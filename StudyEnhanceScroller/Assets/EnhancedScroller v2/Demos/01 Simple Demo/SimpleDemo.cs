...
namespace EnhancedScrollerDemos.SuperSimpleDemo
{
    ...
    public class SimpleDemo : MonoBehaviour, IEnhancedScrollerDelegate
    {
        ...
        void Start()
        {
            ...
            LoadLargeData();
            //scroller.scrollerSnapped += TestGetIndex;
            //  scroller.scrollerScrollingChanged += TestscrollerScrollingChanged;
             scroller.cellViewVisibilityChanged += TestCellViewVisibilityChanged;
        ...
        #region 测试功能
        private void TestCellViewVisibilityChanged(EnhancedScrollerCellView cellView)
        {
            if (cellView.dataIndex==_data.Count-1 &&inson == false)
            {
                count += 20;
                inson = true;
          ...
          //  Debug.Log("dataIndex:" + cellView.dataIndex
        }

        private void  TestGetIndex(EnhancedScroller scroller, int cellIndex, int dataIndex)
        {
            Debug.LogError("cellIndex:"+ cellIndex+"__dataIndex:"+dataIndex);
        }
        private bool inson=false;
        private void TestscrollerScrollingChanged(EnhancedScroller scroller, bool scrolling)//
        {
            if (scrolling==false)//&&inson==false)
            {
                Debug.LogError("Add DataTime");
                //inson = true;
                //count += 20;
                //Addinfo();
            }
